...
> 👨‍💻 **개발자 TIP**: LLM 호출 목록을 만들 때, 반환값을 `str`로만 처리하는 함수에 주목하세요. 구조화된 반환 타입(`dict`, dataclass)이 없는 호출은 성공 판단 로직이 없다는 강력한 신호입니다. 이런 지점에 `@agent_eval` 데코레이터를 붙이면 즉시 TCR·Accuracy 측정이 시작됩니다.

> 📋 **QA 관리자 TIP**: "성공 기준 없음" 항목이 5개 이상이라면, 이식 전 3개월간의 고객 이슈 티켓에서 패턴을 찾으세요. 반복되는 이슈 유형이 어느 LLM 호출과 연결되는지 파악하면, 우선 이식 지점 결정에 드는 분석 시간을 70% 이상 줄일 수 있습니다.
...
> 👨‍💻 **개발자 TIP**: 위험 우선순위 표를 만들 때, Gate E(보안) 항목은 가중치를 따로 두세요. 보안 실패는 사용자 임팩트와 무관하게 배포를 블로킹해야 하기 때문에, Gate E FAIL이 하나라도 있으면 다른 Gate 점수와 관계없이 배포 거부로 처리하는 CI 규칙을 Ch26에서 추가합니다.

> 📋 **QA 관리자 TIP**: "사용자 임팩트 × 발생 빈도" 매트릭스의 상위 3개 항목만 1단계 이식 대상으로 삼으세요. 처음부터 모든 LLM 호출을 측정하려 하면 오히려 아무것도 배포하지 못하는 분석 마비 상태가 됩니다. Ch24의 레벨 0 이식 패턴은 이 3개에만 적용해도 충분히 의미 있는 첫 Gate 판정 결과가 나옵니다.
...
tcr   = metrics["accuracy_metrics"]["tcr"]["tcr"]            # 예: 87.0 (87% 섹션 완료)
acc   = metrics["accuracy_metrics"]["accuracy_scores"]["overall_accuracy"]  # 예: 72.0
...
**기본 예제**: [`Evaluator_Examples/ch22_project_analysis.py`](../../Evaluator_Examples/ch22_project_analysis.py)
...
| 섹션 | 내용 |
|------|------|
| 섹션 1 | 에이전트 토폴로지 3패턴 — 순차 / 루프 포함 / 병렬+집계 시각화 |
| 섹션 2 | LLM 호출 지점 열거 — 입력 타입·출력 형식·성공 기준 기록 |
| 섹션 3 | 기존 품질 지표 발굴 — 측정됨 vs 측정 안 됨 비교 출력 |
| 섹션 4 | 위험 지점 우선순위화 — 외부 입력·루프·비용 기준 분류 |
| 섹션 5 | 측정 단위 추출 — `create_taskresult` + `PerformanceMonitor`로 섹션별 TCR/Accuracy/P95 집계 |
