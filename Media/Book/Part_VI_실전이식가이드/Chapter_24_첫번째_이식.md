...
> 👨‍💻 **개발자 TIP**: 레벨 0 래핑의 유일한 실패 패턴은 래퍼 함수가 기존 함수의 반환 타입을 변경하는 것입니다. `@agent_eval`이 반환값을 tuple로 감싸지 않도록 `EvalMetadata`를 사용할 때는 반드시 `return response, EvalMetadata(...)` 형태로 반환하고, 호출 쪽 코드가 이 tuple을 처리할 수 있는지 먼저 확인하세요.

> 📋 **QA 관리자 TIP**: "기존 코드를 전혀 건드리지 않는다"는 원칙 덕분에 이식 후 회귀 테스트는 기존 테스트 스위트 그대로 통과해야 합니다. 이식 직후 CI에서 기존 유닛 테스트가 실패한다면, 래퍼 함수가 반환 타입을 의도치 않게 바꾼 것이 원인일 가능성이 높습니다.
...
> 👨‍💻 **개발자 TIP**: 위임 어댑터 패턴에서 `try/finally` 블록의 `finally` 절은 반드시 필요합니다. `except` 절 없이 `finally`만 쓰면 예외는 그대로 전파되면서도 오류 케이스가 TCR 실패로 기록됩니다. `except`로 예외를 삼켜버리면 Gate C(신뢰성) 점수가 실제보다 높게 나오므로 주의하세요.

> 📋 **QA 관리자 TIP**: 위임 어댑터 작성 시 원본 파일을 한 줄도 수정하지 않는다는 점이 핵심 장점입니다. 코드 리뷰에서 원본 에이전트 파일에 변경이 없음을 확인하는 것만으로, 이식이 기존 동작에 영향을 주지 않았다는 것을 보증할 수 있습니다.
