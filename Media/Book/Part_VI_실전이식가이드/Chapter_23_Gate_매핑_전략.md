...
> 👨‍💻 **개발자 TIP**: 실패 모드 카탈로그는 10개 이상이면 집중력이 분산됩니다. "발생 시 서비스 중단 여부"와 "3개월 내 발생 가능성" 두 축으로 걸러서 5개 이하로 줄이세요. 나머지는 다음 이식 사이클로 미루는 것이 실용적입니다.

> 📋 **QA 관리자 TIP**: 실패 모드 카탈로그를 스프린트 회고 직후에 작성하면 가장 생생한 데이터를 얻을 수 있습니다. 회고에서 언급된 "아슬아슬하게 넘어간 것"들이 실패 모드 카탈로그의 핵심 항목이 됩니다.
...
<tr style="background:#e8f5e9;">
  ...
  <td><span class="gate-badge" style="background:#2e7d32;">A</span></td>
  <td><span class="config-name">GoalAlignmentConfig(alignment_threshold=0.80)</span></td>
</tr>
...
<tr style="background:#fff3e0;">
  ...
  <td><span class="gate-badge" style="background:#e65100;">C</span></td>
  <td><span class="config-name">FaultToleranceConfig(partial_success_threshold=0.6)</span></td>
</tr>
<tr style="background:#fff3e0;">
...
# GracefulDegradationConfig(quality_floor=0.5)도 사용 가능하지만,
# 여기서는 부분 완성 비율(60%) 기준 측정이 목적이므로 FaultToleranceConfig를 선택한다.
FaultToleranceConfig(
...
> 👨‍💻 **개발자 TIP**: 가중치를 높인 Gate가 FAIL이면 전체 배포가 블로킹됩니다. 처음 설정할 때는 모든 Gate 가중치를 기본값(1.0)으로 두고 2–3 스프린트 데이터를 모은 뒤, 실제로 이슈가 됐던 Gate의 가중치만 올리세요. 처음부터 높은 가중치로 시작하면 배포 블로킹이 너무 잦아져 팀이 평가 도입 자체를 회피하게 됩니다.

> 📋 **QA 관리자 TIP**: Gate 가중치는 비즈니스 우선순위와 직접 연결됩니다. "이 서비스에서 가장 비싼 장애는 무엇인가?"라는 질문에 답하면 가중치 설계가 나옵니다. 예를 들어 금융 서비스라면 Gate E(보안) 가중치를 3.0, 콘텐츠 서비스라면 Gate A(목표달성)를 2.0으로 두는 것이 일반적인 패턴입니다.
