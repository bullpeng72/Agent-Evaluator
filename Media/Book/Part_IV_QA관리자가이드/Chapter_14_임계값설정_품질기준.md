...
```python
# 개념 코드 — PerformanceMonitor 구성 패턴 (QA·Tool Use·보안·RAG 4가지 케이스)
# (실행 가능 전체 예제: Evaluator_Examples/ch14_thresholds.py 참고)
...
load_env()  # .env에서 LLM API 키 로드 (agent-eval init 설정 자동 인식)
monitor = PerformanceMonitor(
    output_dir="results/",
    enable_hallucination_detection=True,
...
def rag_agent(question: str, context: str = "", ground_truth: str = "") -> str:
    # TODO(현업 적용): 아래 Mock을 실제 RAG 체인 호출로 교체하세요.
    #   예) return retrieval_chain.invoke({"question": question, "context": context})
...
```python
# 기반 코드 — QuickEval.generate_gate_config() 데이터 기반 임계값 자동 생성 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch14_thresholds.py 참고)
...
def agent(question: str, ground_truth: str = "") -> str:
    # TODO(현업 적용): 아래 Mock을 실제 LLM 호출로 교체하세요.
...
test_dataset = [("한국의 수도?", "서울"), ("파이썬 창시자?", "귀도 반 로섬"), ("빛의 속도?", "30만 km/s")]
...
```python
# 개념 코드 — 1주차 느슨한 임계값 + 자동 저장 모니터링 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch14_thresholds.py 참고)
...
```python
# 기반 코드 — 2주차 캘리브레이션: replay() + generate_gate_config() 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch14_thresholds.py 참고)
from agent_evaluator import QuickEval
...
```python
# 개념 코드 — 에이전트 유형별 KPI Harness Config 선언 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch14_thresholds.py 참고)
...
```python
# 개념 코드 — Wilson Score 하한 임계값 추정 패턴
# (ch14_thresholds.py에 포함되지 않은 독립 수학 유틸리티 — 직접 실행 가능)
import math
...
```python
# 개념 코드 — 샘플 수 기반 적응형 임계값 결정 패턴
# (ch14_thresholds.py에 포함되지 않은 독립 유틸리티 — 직접 실행 가능)
...
# 2주 캘리브레이션 완료 후 Config 자동 생성 (monitor는 PerformanceMonitor 인스턴스)
...
```python
# 기반 코드 — ch14_thresholds.py 섹션 2 기반 (단순화 버전)
# (실행 가능 전체 예제: Evaluator_Examples/ch14_thresholds.py 참고)
from agent_evaluator import (
...
```python
# 개념 코드 — AnomalyDetector 통계 기반 이상 탐지 패턴
...
monitor_anom = PerformanceMonitor(output_dir="results/", use_korean_tokenizer=True)
...
```python
# 개념 코드 — CostTracker + AdaptivePolicy 비용 제어 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch10_group_g.py 참고)
...
| 섹션 | 내용 | 연관 기능 |
|------|------|-----------|
| 섹션 1 | `QuickEval.generate_gate_config()` | 현재 성능 기반 임계값 자동 생성 (95% 수준) |
| 섹션 2 | `HarnessEvaluationGate` + 엄격 임계값 시뮬레이션 | SLAConfig·ResourceBudgetConfig·InstructionConfig |
| 섹션 3 | `SimpleTaskAlertRule` 실시간 알림 | latency > 2초, accuracy < 0.3 알림 규칙 |
| 섹션 4 | 에이전트 유형별 임계값 프로파일 비교 | QA·RAG·Tool Use·Security·Conversation 5종 |
...
=== 섹션 1: generate_gate_config() — 데이터 기반 임계값 자동 생성 ===
  캘리브레이션 케이스 10건 실행 중...
  생성된 임계값 (95% 수준 기반): tcr=약 53~95%, accuracy=약 25~90%
  저장 위치: results/ch14_gate_config.json

=== 섹션 2: HarnessEvaluationGate — Gate별 임계값 선언 및 판정 ===
  [정상 에이전트] Gate A: score=0.570 → warn   Gate D: score=0.989 → pass
  [엄격 임계값 (p95_ms=100)] Gate A: score=0.420 → fail   Gate D: score=0.494 → fail

=== 섹션 3: SimpleTaskAlertRule — 임계값 기반 알림 ===
  발생한 알림: N건 (랜덤 에이전트 지연·오답 발생 기준)
...
```python
# 개념 코드 — StreamingEvaluator + SimpleTaskAlertRule 슬라이딩 윈도우 알림 패턴
