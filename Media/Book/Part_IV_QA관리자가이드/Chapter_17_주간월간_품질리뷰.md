...
```python
# 개념 코드 — qa_weekly_review.py 주간 리뷰 스크립트 예시
# (실행 가능 전체 예제: Evaluator_Examples/ch17_weekly_review.py 참고)
...
```python
# 기반 코드 — ch17_weekly_review.py 섹션 2 기반 (단순화 버전)
from agent_evaluator import QuickEval
...
```python
# 개념 코드 — QuickEval.ab_test() 반환값 구조
ab_result = eval_this.ab_test(eval_last)

# ab_result["p_value"]                    — 유의성 p-값 (0~1)
# ab_result["t_statistic"]                — t-통계량
# ab_result["sample_sizes"]["self"]        — 이번 주 태스크 수
# ab_result["sample_sizes"]["other"]       — 지난 주 태스크 수
...
- `sample_sizes["self"]`와 `sample_sizes["other"]`를 함께 확인해 한쪽 기간 샘플이 30건 미만이면 결과 신뢰도가 낮다는 점을 감안해야 한다
...
```python
# 개념 코드 — GoldenSetBuilder 주간 고품질 케이스 추출 패턴
...
```python
# 개념 코드 — 골든셋 JSON 파일 월간 케이스 정리 패턴
import json
from datetime import datetime, timedelta
...
def agent(question: str, ground_truth: str = "") -> str:
    # TODO(현업 적용): 아래 Mock 구현을 실제 에이전트 호출로 교체하세요.
...
```python
# 기반 코드 — ch17_weekly_review.py 기반 (Phoenix OTEL 연동 패턴)
...
`monitor.save_to_file()` 또는 `eval_q.save()` 호출 시 JSON과 함께 HTML 보고서가 자동으로 생성된다. 이 HTML 파일은 독립 실행형이므로 이메일에 첨부하거나 팀 위키에 올릴 수 있다.

```python
# 개념 코드 — QuickEval.replay() + save() HTML 월간 보고서 생성 패턴
from agent_evaluator import QuickEval
...
```python
# 개념 코드 — monthly_report.py 월간 보고서 자동화 스크립트 예시
...
```python
# 개념 코드 — PerformanceMonitor.feedback_tracker 주간 암묵적 피드백 집계 패턴
