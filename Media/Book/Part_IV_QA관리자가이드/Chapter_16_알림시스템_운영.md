...
```python
# 개념 코드 — AlertRuleBuilder.when_error() 오류 발생 알림 패턴
...
```python
# 개념 코드 — AlertRuleBuilder.when_tool_calls_exceed() 도구 호출 상한 알림 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch16_alerts.py 참고)
...
```python
# 개념 코드 — QuickEval + alert_rules 조합 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch12_decorators.py 참고)
from agent_evaluator import QuickEval

eval_q = QuickEval("results/")

@eval_q(task_type="qa", alert_rules=[accuracy_rule, latency_rule])
def my_agent(question: str, ground_truth: str = "") -> str:
    return llm.invoke(question)
```

- `QuickEval`의 `@eval_q(...)` 직접 호출 방식에서도 `alert_rules` 파라미터를 동일하게 지원한다
- `@eval_q.qa`, `.rag` 같은 단축 데코레이터에는 `alert_rules`를 전달할 수 없으므로, 알림이 필요하면 `@eval_q(task_type="qa", alert_rules=[...])` 형태를 사용한다
...
```python
# 개념 코드 — 토큰 과다 사용 SimpleTaskAlertRule 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch16_alerts.py 참고)
...
```python
# 개념 코드 — 복합 조건 SimpleTaskAlertRule 패턴 (AND 조건)
# (실행 가능 전체 예제: Evaluator_Examples/ch16_alerts.py 참고)
...
```python
# 개념 코드 — compound_conditions 딕셔너리 방식 복합 조건 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch16_alerts.py 참고)
...
# dry_run: 핸들러 실행 없이 조건만 검사 → "would_fire": True면 알림 발동할 것
assert rule.dry_run(low_accuracy_task)["would_fire"] == True
...
assert rule.dry_run(high_accuracy_task)["would_fire"] == False
...
```python
# 개념 코드 — class_level_cooldown 공유 쿨다운 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch16_alerts.py 참고)
...
```python
# 개념 코드 — 환경변수 기반 Slack 핸들러 구현 패턴
# (ch16_alerts.py는 Mock 핸들러를 사용하며 이 함수를 포함하지 않음)
import os
import json
...
```python
# 개념 코드 — 5-규칙 프로덕션 알림 설정 패턴 (create_slack_handler/create_email_handler 별도 구현 필요)
# (실행 가능 기반 예제: Evaluator_Examples/ch16_alerts.py 섹션 3~4 참고)
import os
...
```python
# 개념 코드 — 대시보드 API explain_event 호출 패턴
# (ch16_alerts.py에 없음 — agent-eval dashboard 실행 후 HTTP API 사용 예)
...
```python
# 개념 코드 — AdaptivePolicy + CostTracker 예산 자동 제어 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch10_group_g.py 참고 — AdaptivePolicy·CostTracker는 ch16에 없음)
...
```python
# 개념 코드 — CI 알림 규칙 단위 테스트 패턴 (dry_run 활용)
# (실행 가능 전체 예제: Evaluator_Examples/ch16_alerts.py 섹션 3 dry_run 부분 참고)
...
def test_alert_rules():
    ...
    assert rule.dry_run(bad_task)["would_fire"], "낮은 accuracy에서 알림이 발동해야 함"
    ...
    assert not rule.dry_run(good_task)["would_fire"], "높은 accuracy에서 알림이 발동하면 안 됨"
```
...
| ch16_alerts | 섹션 5 | `AlertEngine` + `AlertRule` 고급 알림 엔진 | 슬라이딩 윈도우 기반 집계 조건 알림 |
| ch16_alerts | 섹션 6 | `eval_context` + `alert_rules` | 데코레이터 없는 컨텍스트 기반 알림 |
