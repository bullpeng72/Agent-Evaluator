...
<div class="pipeline-row">
  ...
  <div class="phase-card" style="background:#e8f5e9;border:2px solid #66bb6a;">
    ...
    <ul class="step-list">
      <li class="step-item" style="color:#1b5e20;"><span class="step-icon">🏷️</span><div><div>@eval_q.qa 데코레이터 적용</div><div>실행마다 자동 채점</div><span class="step-code" style="background:#c8e6c9;color:#1b5e20;">@eval_q.qa</span></div></li>
      <li class="step-item" style="color:#1b5e20;"><span class="step-icon">🧪</span><div><div>소수 케이스 로컬 실행</div><div>ground_truth 포함</div></div></li>
      <li class="step-item" style="color:#1b5e20;"><span class="step-icon">💾</span><div><div>results/ 결과 JSON 생성</div><span class="step-code" style="background:#c8e6c9;color:#1b5e20;">eval_q.save()</span></div></li>
      <li class="step-item" style="color:#1b5e20;"><span class="step-icon">🔍</span><div><div>TCR · 정확도 즉시 점검</div><span class="step-code" style="background:#c8e6c9;color:#1b5e20;">eval_q.summary()</span></div></li>
  ...
  <div class="phase-card" style="background:#e3f2fd;border:2px solid #42a5f5;">
    ...
    <ul class="step-list">
      ...
      <li class="step-item" style="color:#0d47a1;"><span class="step-icon">💾</span><div><div>CI 결과 JSON 저장</div><span class="step-code" style="background:#bbdefb;color:#0d47a1;">eval_q.save()</span></div></li>
...
```python
# 개념 코드 — 개발 단계 QuickEval 로컬 검증 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch21_pipeline.py 참고)
# dev/run_dev.py
...
if __name__ == "__main__":
    ...
    eval_q.save()
    print(eval_q.summary())
```

**2단계: CI**

```python
# 개념 코드 — CI 단계 QuickEval 평가 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch21_pipeline.py 참고)
...
```python
# 기반 코드: Evaluator_Examples/ch21_pipeline.py — 운영 단계 데코레이터 + 알림 통합
# production/agent.py
import os
...
```python
# 기반 코드: Evaluator_Examples/ch21_pipeline.py — 4단계 주간 회귀 + 트렌드 분석
...
def run_weekly_regression():
    ...
    try:
        ...
        print(f"TCR 변화: {comparison['delta'].get('tcr', 0):+.1f}%")
...
```python
# 개념 코드 — 1인 개발자 QuickEval 최소 시작 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch21_pipeline.py 참고)
# evaluate.py — 5줄로 시작
from agent_evaluator import QuickEval
...
eval_q.save()
eval_q.gate(tcr=80, accuracy=70)  # 실패 시 sys.exit(1)
```
...
- `python evaluate.py`에서 `eval_q.gate()`를 직접 호출하거나 별도 `agent-eval gate` 스텝으로 분리하는 두 방식 모두 동작한다
...
# 50개 케이스 수집
existing_qa_pairs = [{"question": "질문1", "answer": "정답1"}, {"question": "질문2", "answer": "정답2"}]
...
- 50개 케이스부터 시작해 `eval_q.save()`로 결과 파일을 저장하면 골든 데이터셋 빌드의 소스가 된다
...
```python
# 개념 코드 — 소규모 팀 알림 규칙 설정 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch21_pipeline.py 참고)
from agent_evaluator import SimpleTaskAlertRule, PerformanceMonitor, agent_eval
...
@agent_eval(monitor, task_type="qa", alert_rules=[accuracy_alert])
def agent(question: str, ground_truth: str = "") -> str:
    # TODO(현업 적용): 아래 Mock 구현을 실제 LLM 호출로 교체하세요.
    #   예) return client.chat.completions.create(model="gpt-5-nano",
    #        messages=[{"role":"user","content":question}]).choices[0].message.content
...
```python
# 기반 코드: Evaluator_Examples/ch21_pipeline.py — 운영 단계 5-규칙 알림 설정
...
# TODO(현업 적용): slack_alert, pagerduty_alert, security_alert를 실제 알림 함수로 교체하세요.
def slack_alert(msg, tr): pass       # Slack 웹훅 연동
def pagerduty_alert(msg, tr): pass   # PagerDuty 연동
def security_alert(msg, tr): pass    # 보안팀 알림
...
```python
# 기반 코드: Evaluator_Examples/ch21_pipeline.py — 4단계 compare() 트렌드 분석
from agent_evaluator import QuickEval
...
```python
# 개념 코드 — 멀티모달 에이전트 EvalMetadata 활용 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch21_pipeline.py 참고)
...
def vision_agent(question: str, image_path: str, ground_truth: str = "") -> tuple:
    # TODO(현업 적용): call_vision_llm을 실제 멀티모달 LLM 호출로 교체하세요.
    response = f"이미지 분석 결과: {question}"
...
```python
# 개념 코드 — DSPy 프롬프트 최적화 연동 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch21_pipeline.py 참고)
from agent_evaluator import QuickEval, agent_eval, PerformanceMonitor
import dspy
...
def dspy_agent(question: str, ground_truth: str = "") -> str:
    # TODO(현업 적용): dspy_program을 실제 DSPy 프로그램으로 교체하세요.
    return f"DSPy 답변: {question}"
```
...
```python
# 개념 코드 — BaseTracker 커스텀 트래커 구현 패턴
...
| 드리프트 유형 | 정의 | 감지 신호 (Harness Gate) | agent-eval 지표 |
|------------|------|--------------------------|----------------|
...
```python
# 개념 코드 — RunTrendAnalyzer 드리프트 감지 패턴
...
report = analyzer.analyze()  # RunTrendReport 객체 반환

# 드리프트 판정 기준 (MetricTrend.slope: run당 변화율)
drift_signals = []

acc_slope = report.accuracy_trend.slope if report.accuracy_trend else 0
if acc_slope < -0.02:    # 주당 2% 이상 정확도 하락
    drift_signals.append(("concept_drift", "AccuracyEvaluator", acc_slope))

tcr_slope = report.tcr_trend.slope if report.tcr_trend else 0
if tcr_slope < -0.03:          # 주당 3% 이상 TCR 하락
    drift_signals.append(("data_drift", "TaskCompletionTracker", tcr_slope))

lat_slope = report.latency_trend.slope if report.latency_trend else 0
if lat_slope > 0.5:   # P95 지연 주당 0.5초 이상 상승
    drift_signals.append(("operational_drift", "LatencyTracker", lat_slope))
...
```python
# 개념 코드 — Wilson Score 기반 임계값 재보정 패턴
...
```python
# 개념 코드 — RunTrendAnalyzer 자기개선 루프 Stage 1 패턴
from agent_evaluator.cli.trend import RunTrendAnalyzer
...
print(f"TCR 추세: {tcr_slope:+.3f}/week")
print(f"정확도 추세: {acc_slope:+.3f}/week")
print(f"P95 지연 추세: {lat_slope:+.3f}s/week")

# 개선 필요 여부 판정
needs_improvement = (
    tcr_slope < -0.01 or      # TCR 주당 1% 이상 하락
    acc_slope < -0.01          # 정확도 주당 1% 이상 하락
)
```

### Stage 2 — 원인 귀속 (Gate별 Tracker 드릴다운)

```python
# 개념 코드 — harness_groups 드릴다운 패턴
...
```python
# 개념 코드 — RunTrendAnalyzer 자기개선 KPI 측정 패턴
from agent_evaluator.cli.trend import RunTrendAnalyzer

# 개선 전 기준선
before = RunTrendAnalyzer("results/before_improvement/", window=14).analyze()
# 개선 후 측정
after = RunTrendAnalyzer("results/after_improvement/", window=14).analyze()

# last_val = 분석 윈도우의 마지막(최신) 값
before_tcr = before.tcr_trend.last_val if before.tcr_trend else 0
after_tcr  = after.tcr_trend.last_val  if after.tcr_trend  else 0
before_acc = before.accuracy_trend.last_val if before.accuracy_trend else 0
after_acc  = after.accuracy_trend.last_val  if after.accuracy_trend  else 0

tcr_improvement      = after_tcr - before_tcr
accuracy_improvement = after_acc - before_acc

print(f"개선 효과:")
print(f"  TCR: {before_tcr:.1%} → {after_tcr:.1%} ({tcr_improvement:+.1%})")
print(f"  정확도: {before_acc:.1%} → {after_acc:.1%} ({accuracy_improvement:+.1%})")
```
...
```python
# 기반 코드 — ch21_pipeline.py §1단계 QuickEval (단순화)
# 이것만으로도 충분한 시작이다
from agent_evaluator import QuickEval
...
```python
# 기반 코드 — ch21_pipeline.py §3단계 데코레이터 + 알림 (단순화)
...
def production_agent(question: str, context: str = "", ground_truth: str = "") -> tuple:
    ...
    return response, EvalMetadata(
        tool_calls=[{"tool_name": "retriever"}, {"tool_name": "llm"}],
...
```python
# 기반 코드 — ch21_pipeline.py §4단계 골든셋 + 추세 분석 + CI 게이팅 (단순화)
...
```python
# 기반 코드 — ch20_deployment.py Gate별 점수 비교 (단순화)
...
```python
# 기반 코드 — ch20_deployment.py 배포 결정 자동화 (단순화)
