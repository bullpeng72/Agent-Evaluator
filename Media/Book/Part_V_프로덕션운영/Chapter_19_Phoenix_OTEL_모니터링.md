...
```python
# 개념 코드 — setup_otel() 올바른 초기화 순서 패턴
...
#   arize-phoenix>=15.4.0 / opentelemetry-sdk>=1.20.0 기본 설치에 포함
setup_otel(
...
```python
# 개념 코드 — QuickEval + setup_otel() 완전한 연동 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py 참고)
...
monitor = PerformanceMonitor(output_dir="results/", use_korean_tokenizer=True)  # 여기서 이미 OTEL 없이 초기화됨
...
```python
# 개념 코드 — enable_otel_child_spans 워터폴 자식 스팬 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py 참고)
...
def multi_tool_agent(question: str, ground_truth: str = "") -> tuple:
    # TODO(현업 적용): 아래 API 호출을 실제 서비스 구현으로 교체하세요.
...
```python
# 기반 코드 — 섹션 3 GoldenSetBuilder.push_to_phoenix() 패턴 (extract 단계 확장)
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py — 섹션 3 참고)
...
```python
# 개념 코드 — EvalMetadata llm.prompts Playground 연동 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py — 섹션 2 참고)
from agent_evaluator import setup_otel, PerformanceMonitor
from agent_evaluator import agent_eval, EvalMetadata
...
```python
# 기반 코드 — 섹션 4 Phoenix REST API 프롬프트 등록 패턴 (페이로드 구조 단순화)
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py — 섹션 4 참고)
import requests
...
```python
# 개념 코드 — Grafana Tempo 엔드포인트 연동 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py 참고)
...
```python
# 개념 코드 — Jaeger/Zipkin 엔드포인트 연동 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py 참고)
# Jaeger
setup_otel(
...
- **arize-phoenix 버전**: `>=15.4.0`. `opentelemetry-sdk>=1.20.0` 및 `opentelemetry-exporter-otlp-proto-http>=1.20.0`이 기본 설치에 포함된다. `agent-eval monitor --check`으로 설치 여부를 즉시 확인할 수 있다.
...
```python
# 기반 코드 — Phoenix 조건부 연결 + PerformanceMonitor 초기화 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py 참고)
import socket
from agent_evaluator import setup_otel, PerformanceMonitor
...
- `setup_otel()`은 반드시 `PerformanceMonitor(, use_korean_tokenizer=True)` 생성 **이전**에 호출해야 한다 — 순서가 바뀌면 스팬이 Phoenix로 전송되지 않는다
...
```python
# 기반 코드 — 섹션 2 Playground llm.prompts + Datasets 연동 스팬 전송 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py — 섹션 2 참고)
...
**실행 결과 (v0.9.7 기준, Phoenix 미실행 mock 모드)**
...
=== 섹션 1: Phoenix Tracing + Annotations ===
  [qa                    ] span.kind=LLM       acc=0.82
  [tool_use              ] span.kind=TOOL      acc=0.00
  [information_retrieval ] span.kind=RETRIEVER acc=0.00
  [code_generation       ] span.kind=LLM       acc=1.00
  [planning              ] span.kind=AGENT     acc=0.00

=== 섹션 3: Phoenix Datasets 탭 ===
  고품질 케이스 2건 추출 (score≥0.6)
...
=== 외부평가 탭 데모 데이터 주입 (목업) ===
  ...
  → agent-eval dashboard 실행 후 '외부평가' 탭 확인
```

> **`setup_otel()` 순서 엄수**: `setup_otel(endpoint="http://localhost:6006")`은 반드시 `PerformanceMonitor(..., use_korean_tokenizer=True)` 또는 `QuickEval(...)` 생성 전에 호출해야 한다. 순서를 틀리면 스팬이 Phoenix에 전송되지 않는다. `ch19_phoenix.py` 섹션 1의 코드 순서를 템플릿으로 사용한다.
...
```python
# 기반 코드 — 섹션 추가 DeepEvalAdapter + RagasAdapter 단건 평가 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py — 섹션 추가 참고)
...
```python
# 기반 코드 — Phoenix 조건부 연결 + CI/CD Gate 판정 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch19_phoenix.py 참고; Gate 판정은 ch18_cicd_gate.py)
import socket, sys
