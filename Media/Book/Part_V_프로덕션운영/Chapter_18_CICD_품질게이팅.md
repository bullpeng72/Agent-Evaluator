...
```python
# 개념 코드 — QuickEval 기반 자동 gate 설정 생성 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch18_cicd_gate.py 참고)
...
```python
# 개념 코드 — CI/CD 환경 QuickEval 평가 스크립트 패턴
# (ch18_cicd_gate.py는 QuickEval 없이 PerformanceMonitor+agent_eval 직접 사용 — 차이 참고)
...
```python
# 개념 코드 — 환경 변수 기반 eval_q.gate() 분기 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch18_cicd_gate.py 참고)
import os
from agent_evaluator import QuickEval
...
```python
# 개념 코드 — 게이트 실패 원인 분석 패턴 (낮은 정확도 케이스 추출)
# (ch18_cicd_gate.py는 이 패턴을 포함하지 않음)
import json
...
```python
# 개념 코드 — CI_CHANGE_SOURCE 환경변수 기반 게이팅 프로파일 패턴
# (ch18_cicd_gate.py는 QuickEval·GATE_PROFILES를 포함하지 않음)
import os
...
eval_q = QuickEval("results/")
eval_q.gate(tcr=profile["tcr"], accuracy=profile["accuracy"])
```
...
```python
# 개념 코드 — HarnessEvaluationGate CI/CD 통합 패턴
# (ch18_cicd_gate.py 기반이지만 production_agent 등 일부 요소는 포함하지 않음)
...
# Harness Gate 위반 여부: extra_metrics.harness_groups에서 FAIL 항목 집계
harness_groups = (report.to_dict().get("extra_metrics") or {}).get("harness_groups", {})
violations = [k for k, v in harness_groups.items() if v.get("gate", "").upper() == "FAIL"]
...
# (ch18_cicd_gate.py는 evaluation_session을 사용하지 않음 — PerformanceMonitor 직접 사용)
...
```python
# 기반 코드 — Evaluator_Examples/ch18_cicd_gate.py — Harness 7개 Gate CI/CD 게이팅 (_group_a/b 에이전트 기반)
# (_print_gate_and_exit은 ch18의 _print_gate를 단순화한 패턴)
...
```python
# 개념 코드 — Gate E FAIL + CI/CD 차단 패턴
# (ch18_cicd_gate.py의 ComplianceConfig/ThreatSeverityConfig 기반이나 pii_leaking_agent·HarnessEvaluationGate는 포함하지 않음)
import sys
from agent_evaluator import (
...
monitor_e = PerformanceMonitor(output_dir="results/", enable_security_metrics=True, use_korean_tokenizer=True)
...
# gate.evaluate() 반환: {passed, groups, violations, summary}
# groups: {"E": {"score": float, "status": str, "passed": bool, "details": {}}}
if not result.get("groups", {}).get("E", {}).get("passed", True):
...
```python
# 개념 코드 — 두 버전 Harness Gate 점수 비교 기반 배포 결정 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch20_deployment.py 참고)
import sys
