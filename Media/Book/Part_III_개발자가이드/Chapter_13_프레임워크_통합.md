# Chapter 13. 24개 프레임워크 통합

이 챕터에서 배우는 것: Agent-Evaluator SDK가 LangChain, CrewAI, AutoGen, DSPy, Anthropic, OpenAI 등 24개 프레임워크와 어떻게 통합되는지를 이해한다. `framework=` 파라미터 하나가 내부적으로 어떤 자동 추출 메커니즘을 작동시키는지, 각 프레임워크의 설치 방법과 주요 자동 추출 항목을 파악하고, 팀 상황에 맞는 프레임워크를 선택하는 기준을 배운다. OpenAI Agents SDK·Google ADK·Claude Agent SDK 등 공식 에이전트 프레임워크 3종이 새로 추가되었다.
...
이 과정이 `_FRAMEWORK_ADAPTERS` 레지스트리에 등록된 24개 어댑터 함수를 통해 이루어진다. 각 어댑터는 해당 프레임워크 고유의 응답 구조를 파악하고 SDK 내부 형식으로 변환한다. 변환된 데이터는 자동으로 Harness Gate 집계에 포함되므로, 에이전트 코드를 한 줄도 바꾸지 않고 Gate A–G 판정을 받을 수 있다.
...
`framework=`를 명시하지 않아도 SDK 내부 자동 감지 메커니즘이 응답 객체의 속성을 분석해 프레임워크를 자동으로 판별한다:

```python
# 개념 코드 — framework= 생략 시 자동 감지 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py 참고)
...
> **주의 — 신규 3개 프레임워크는 자동 감지되지 않는다.** `openai_agents`/`google_adk`/`claude_agent_sdk`는 기존 프레임워크(특히 `claude_agent_sdk`의 `AssistantMessage`가 raw Anthropic `Message`와 속성 구조가 겹치는 부분)와 자동 감지 로직이 오탐 충돌할 위험이 검증되지 않아, 의도적으로 자동 감지 대상에서 제외했다. 이 3개는 반드시 `framework=` 파라미터를 명시해야 한다.

### get_framework_info() — 어댑터 메타데이터 조회

```python
# 개념 코드 — get_framework_info() 어댑터 메타데이터 조회 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py 참고)
from agent_evaluator import get_framework_info
...
#   "is_installed": True,
#   "supports_chain_steps": True
# }
```

- `get_framework_info(name)`는 해당 어댑터가 추출하는 항목, 설명, 설치 여부를 dict로 반환한다
- `"extracts"` 키의 목록이 비어있으면 해당 프레임워크에서 메타데이터 자동 추출이 제한된다
...
> 2. `PerformanceMonitor(output_dir="results/", use_korean_tokenizer=True)` 생성
...
```python
# 개념 코드 — LangChain 실제 통합 패턴 (실제 langchain 패키지 필요)
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py — 섹션 1 참고)
...
### LCEL 툴콜링 — AgentExecutor 없이 직접 반환하는 패턴

`AgentExecutor`를 아예 쓰지 않고 `model.bind_tools(...).invoke(...)`로 도구 바인딩 모델을 직접 호출하는 것이 현재 LangChain이 권장하는 방식이다. 이 경우 함수는 `dict`가 아니라 `AIMessage` 객체를 직접 반환한다 — 어댑터가 이 경로도 자동으로 처리한다:

```python
# 개념 코드 — LCEL 툴콜링 직접 반환 패턴 (AgentExecutor 불필요)
from langchain_openai import ChatOpenAI
...
@agent_eval(monitor, task_type="tool_use", framework="langchain")
def lcel_agent(question: str, ground_truth: str = ""):
    return llm.invoke(question)  # AIMessage 객체 그대로 반환 — dict로 감쌀 필요 없음

lcel_agent("서울의 오늘 날씨는?", ground_truth="맑음, 15도")
```

- `AIMessage.tool_calls`(`{"name", "args", "id"}` 리스트)에서 도구 호출을, `AIMessage.usage_metadata`(dict)에서 토큰 수를 직접 추출한다
- `AgentExecutor.invoke()`의 `intermediate_steps` 경로와 이 경로 중 어느 쪽이든 자동으로 판별되므로, 두 스타일을 같은 프로젝트에서 섞어 써도 `framework="langchain"` 하나로 충분하다
...
```python
# 개념 코드 — LangGraph @langgraph_eval 전용 데코레이터 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py — 섹션 2 참고)
...
```python
# 개념 코드 — CrewAI @crewai_eval 전용 데코레이터 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py — 섹션 3 참고)
...
```python
# 개념 코드 — CrewAI 토큰 수 EvalMetadata 수동 주입 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py — 섹션 3 참고)
@crewai_eval(monitor, task_type="tool_use")
...
```python
# 개념 코드 — AutoGen @autogen_eval 전용 데코레이터 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py — 섹션 4 참고)
import asyncio
...
```python
# 개념 코드 — DSPy @dspy_eval 전용 데코레이터 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py 참고)
import dspy
from agent_evaluator import PerformanceMonitor
...
```python
# 개념 코드 — PydanticAI @pydanticai_eval 전용 데코레이터 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py 참고)
...
```python
# 개념 코드 — Anthropic API framework="anthropic" 통합 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py 참고)
import anthropic
from agent_evaluator import PerformanceMonitor
from agent_evaluator import agent_eval
...
```python
# 개념 코드 — OpenAI API framework="openai" 통합 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py 참고)
...
- **Responses API도 같은 어댑터로 처리된다**: `client.responses.create(...)`가 반환하는 `Response` 객체는 `choices`가 아니라 `output` 리스트를 쓰는 구조라 완전히 다른 모양이지만, 어댑터가 `output[].type == "function_call"` 아이템에서 도구 호출을, `usage.input_tokens`/`usage.output_tokens`에서 토큰 수를 자동으로 찾아낸다 — `framework="openai"`만 지정하면 Chat Completions와 Responses API 둘 다 그대로 동작한다

**자동 추출**: `choices[0].message.tool_calls` (Chat Completions) 또는 `output[].function_call` (Responses API), `usage.total_tokens` / `usage.prompt_tokens` / `usage.completion_tokens` (또는 `input_tokens`/`output_tokens`).

### Gemini

```python
# 개념 코드 — Gemini API framework="gemini" 통합 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py 참고)
import os
...
## 13.7 공식 에이전트 프레임워크 SDK — OpenAI Agents SDK · Google ADK · Claude Agent SDK

지금까지 다룬 프레임워크는 대부분 서드파티(LangChain, CrewAI 등)이거나 직접 LLM API 호출(Anthropic, OpenAI, Gemini)이었다. 이 절의 세 가지는 각 벤더가 **공식적으로 내놓은 에이전트 오케스트레이션 SDK**라는 공통점이 있다 — OpenAI의 Swarm 후속작, Google의 멀티에이전트 프레임워크, Anthropic의 에이전틱 루프 SDK다. 셋 다 원시 API 응답과는 다른 자체 객체 구조를 쓴다.

> **공통 제약**: 세 프레임워크 모두 §13.1의 자동 감지(12개 속성) 대상이 아니다 — `framework=` 파라미터를 반드시 명시해야 한다.
...
@agent_eval(monitor, task_type="tool_use", framework="openai_agents")
def openai_agents_agent(question: str, ground_truth: str = ""):
    return Runner.run_sync(agent, question)  # RunResult 객체 전체 반환

openai_agents_agent("서울의 인구는?", ground_truth="약 950만 명")
```

- `Runner.run()`/`run_sync()`가 반환하는 `RunResult`를 그대로 반환해야 한다 — Chat Completions의 `choices[0].message`와는 다른 구조다
- `new_items`(`RunItem` 리스트) 중 `ToolCallItem`에서 `tool_name`/`call_id`(SDK가 제공하는 편의 프로퍼티)를 추출하고, `raw_responses`(세션 중 모델 호출마다 1건씩 쌓이는 `ModelResponse` 리스트)의 `usage`를 전부 합산해 `tokens_used`를 계산한다

**자동 추출**: `new_items`의 `ToolCallItem` (도구 호출), `raw_responses[].usage` 합산 (토큰).
...
```python
# 개념 코드 — Google ADK 통합 패턴
from agent_evaluator import PerformanceMonitor, agent_eval

monitor = PerformanceMonitor(output_dir="results/", use_korean_tokenizer=True)

@agent_eval(monitor, task_type="tool_use", framework="google_adk")
def adk_agent(question: str, ground_truth: str = ""):
    last_event = None
    for event in runner.run(user_id="u1", session_id="s1", new_message=question):
        last_event = event
    return last_event  # 세션 마지막 Event 객체 반환

adk_agent("서울의 날씨는?", ground_truth="맑음")
```

- `runner.run()`/`run_async()`는 세션 중 여러 `Event`를 순차로 yield하는 제너레이터다 — `@agent_eval`은 함수가 단일 값을 반환한다고 가정하므로, 호출자가 스트림을 직접 순회하며 **마지막(또는 도구 호출이 있는) `Event`**를 반환해야 한다
- `Event`는 `LlmResponse`를 상속해 `get_function_calls()`(Gemini와 동일한 `google.genai.types.FunctionCall` 타입)와 `usage_metadata`(`prompt_token_count`/`candidates_token_count` — 기존 `gemini` 어댑터와 필드명이 같다)를 그대로 제공한다

**자동 추출**: `get_function_calls()` (도구 호출), `usage_metadata` (토큰, gemini 어댑터와 동일 필드).
...
```python
# 개념 코드 — Claude Agent SDK 통합 패턴
from agent_evaluator import PerformanceMonitor, agent_eval

monitor = PerformanceMonitor(output_dir="results/", use_korean_tokenizer=True)

@agent_eval(monitor, task_type="tool_use", framework="claude_agent_sdk")
async def claude_agent_sdk_agent(question: str, ground_truth: str = ""):
    last_assistant_msg = None
    async for message in query(prompt=question):
        if type(message).__name__ == "AssistantMessage":
            last_assistant_msg = message
    return last_assistant_msg  # 도구 호출은 AssistantMessage에서 추출

import asyncio
asyncio.run(claude_agent_sdk_agent("파일을 검색해줘", ground_truth="검색 완료"))
```

- `query(...)`는 `AssistantMessage`/`ResultMessage` 등 여러 타입을 순차 yield하는 비동기 스트림이다. 호출자가 다음 중 하나를 골라 반환해야 한다:
  - **`AssistantMessage`**: `content`의 `ToolUseBlock`에서 도구 호출을 추출한다 (`usage`는 그 메시지 1건분)
  - **`ResultMessage`**: 세션 전체 누적 `usage`/`total_cost_usd`/`num_turns`를 추출한다 (`extra`로 노출되지만, 도구 호출 상세는 없다)
- 도구 호출과 세션 총 토큰을 모두 원한다면, 스트림을 순회하며 직접 병합해 `EvalMetadata`로 반환하는 편이 이 어댑터 하나만 거치는 것보다 정확하다

**자동 추출**: `AssistantMessage.content`의 `ToolUseBlock` (도구 호출) 또는 `ResultMessage.usage`/`total_cost_usd` (세션 총 토큰·비용).
...
- **자동 감지 기본 활성화**: `framework=` 미지정 시 SDK 내부 메커니즘이 12개 속성을 분석해 프레임워크를 자동 판별한다. 단, 정확도를 위해 명시 권장.
...
- **`langchain` 어댑터는 두 반환 형태를 모두 지원한다**: `AgentExecutor.invoke()`의 `intermediate_steps` dict뿐 아니라, `AgentExecutor` 없이 `model.bind_tools(...).invoke(...)`가 직접 반환하는 `AIMessage`도 자동 판별된다.
- **`openai` 어댑터는 Chat Completions·Assistants API·Responses API 셋 다 처리한다** — `client.responses.create(...)`가 반환하는 `output[]` 구조도 같은 `framework="openai"`로 동작한다.
- **공식 에이전트 SDK 3종(OpenAI Agents SDK·Google ADK·Claude Agent SDK)이 신규 추가**됐다 — 단, 세 개 모두 자동 감지 대상이 아니므로 `framework=`를 반드시 명시해야 하고, Google ADK/Claude Agent SDK는 세션 스트림 중 마지막(또는 도구 호출이 있는) 메시지를 골라 반환해야 한다.
...
```python
# 기반 코드 — 섹션 5 크로스 프레임워크 파이프라인 패턴 (응답 구조 인라인 전개)
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py — 섹션 5 참고)
from types import SimpleNamespace
from agent_evaluator import PerformanceMonitor, agent_eval
...
for task in ["경제 위기 예측 리포트 작성", "신제품 출시 전략 수립", "기술 트렌드 분석"]:
...
=== [참고] PydanticAI 실제 통합 예시 (Real-world Bridge) ===
  ...
  ✅ PydanticAI Duck Typing 어댑터 작동 확인 (usage.request_tokens 자동 추출)

=== 섹션 1: LangChain 어댑터 ===
  LangChain 3건 완료 (tool_calls·chain_steps 자동 추출)

=== 섹션 2: LangGraph 어댑터 ===
  LangGraph 2건 완료 (graph_traversal·state_transitions 자동 추출)

=== 섹션 3: CrewAI 어댑터 ===
  CrewAI 2건 완료 (agent_interactions·tasks_output 자동 추출)

=== 섹션 4: AutoGen 어댑터 ===
  AutoGen 2건 완료 (conversation_turns·agent_interactions 자동 추출)

=== 섹션 5: 크로스 프레임워크 파이프라인 ===
  흐름: LangGraph(라우팅) → LangChain(검색) → CrewAI(생성)
  ✅ '경제 위기 예측 리포트 작성' 파이프라인 완료 (3단계)
  ✅ '신제품 출시 전략 수립' 파이프라인 완료 (3단계)
  ✅ '기술 트렌드 분석' 파이프라인 완료 (3단계)

=== 섹션 6: 프레임워크별 배치 비교 ===
  LangChain 배치 3건 / CrewAI 배치 3건 완료

=== 최종 리포트 ===
  총 태스크: 25건  TCR: 52.0%  평균 정확도: 0.26%
  ℹ  정확도가 낮게 측정되는 것은 정상입니다.
     프레임워크 어댑터 예제는 실제 LLM 호출 없이 mock 응답을 사용하며,
     ground_truth와 mock 응답의 토큰 오버랩이 매우 낮아 수 % 수준으로 나타납니다.
...
```python
# 기반 코드 — ch13_frameworks.py 섹션 1 기반, Phoenix OTEL 스팬 전송 추가
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py — 섹션 1 참고)
import socket
from types import SimpleNamespace
