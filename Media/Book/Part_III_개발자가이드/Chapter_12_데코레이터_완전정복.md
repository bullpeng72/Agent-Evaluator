...
```python
# 개념 코드 — 데코레이터 없는 방식 구현 패턴 (PerformanceMonitor·TaskResult 수동 관리)
# (실행 가능 전체 예제: Evaluator_Examples/ch12_decorators.py 참고)
...
지원 프레임워크(24개): `langchain`, `langgraph`, `crewai`, `autogen`, `dspy`, `pydanticai`, `anthropic`, `openai`, `gemini`, `llamaindex`, `haystack`, `vertexai`, `ollama`, `cohere`, `groq`, `mistral`, `bedrock`, `smolagents`, `semantic_kernel`, `vllm`, `huggingface`, `openai_agents`, `google_adk`, `claude_agent_sdk`(신규 3종 — 상세는 Chapter 13 §13.7 참고)

`framework=` 파라미터를 지정하면 해당 SDK 응답 객체에서 토큰 수, 도구 호출 기록 등을 자동 추출한다. 이때 함수가 SDK 응답 **객체 전체**를 반환해야 한다는 점이 핵심이다:

```python
# 개념 코드 — framework= 파라미터별 응답 객체 반환 패턴
...
```python
# 개념 코드 — LangChain duck-typing 어댑터 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch13_frameworks.py 참고)
...
```python
# 개념 코드 — rag_mode=True RAG 평가 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch12_decorators.py, 섹션 8 QuickEval.for_rag 참고)
@agent_eval(
    monitor,
...
```python
# 개념 코드 — SecurityConfig 임시 활성 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch08_group_e.py 참고)
from agent_evaluator import agent_eval, SecurityConfig
...
- 전체 monitor에 항상 보안 지표를 켜려면 `PerformanceMonitor(enable_security_metrics=True, use_korean_tokenizer=True)`를 사용한다
...
@agent_eval(
    monitor,
    task_type="qa",
    llm_judge=LLMJudgeConfig(model="gpt-5-nano"),  # 채점에 사용할 LLM
)
...
```python
# 개념 코드 — 모든 파라미터 조합 패턴 (AlertRuleBuilder 별도 import 필요)
# (실행 가능 전체 예제: Evaluator_Examples/ch12_decorators.py 섹션 5 참고)
from agent_evaluator import PerformanceMonitor, SimpleTaskAlertRule, AlertRuleBuilder
from agent_evaluator import agent_eval, LLMJudgeConfig, RetryConfig
...
@agent_eval(
    ...
    llm_judge=LLMJudgeConfig(model="gpt-5-nano"),  # LLM Judge 채점 활성
...
```python
# 개념 코드 — EvalDecorator 공통 설정 재사용 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch12_decorators.py 섹션 8 참고)
from agent_evaluator import EvalDecorator, LLMJudgeConfig
from agent_evaluator import PerformanceMonitor, AlertRuleBuilder

monitor = PerformanceMonitor(output_dir="results/", use_korean_tokenizer=True)

# 공통 설정을 한 번만 정의
eval_d = EvalDecorator(
    monitor,
    framework="openai",
    llm_judge=LLMJudgeConfig(model="gpt-5-nano"),
    alert_rules=[
...
def qa_agent(question: str, ground_truth: str = "") -> str:
    return client.chat.completions.create(model="gpt-5-nano",
        messages=[{"role": "user", "content": question}])
...
judge_eval = QuickEval.for_llm_judge("results/", model="gpt-5-nano")
```
...
- `eval_q.save()`는 `results/quickeval.json`과 `quickeval.html`을 함께 생성한다
- `eval_q.gate(tcr=85, accuracy=70)`은 기준 미달 시 `sys.exit(1)`을 호출해 CI/CD 파이프라인을 중단시킨다
- `eval_q.summary()`는 accuracy, p95_latency, total_cost_usd 등 핵심 지표를 dict로 반환한다
...
def add_model_info(task_result):
    ...
    new_extra = {**task_result.extra, "model": "gpt-5-nano", "version": "2.0"}
...
from agent_evaluator import agent_eval, RetryConfig
...
| `framework="langchain"` | tool_calls · chain_steps · tokens_used 자동 추출 | 24개 프레임워크 지원 |
...
#### 4. 보안 에이전트
```python
# 개념 코드 — SecurityConfig 보안 에이전트 패턴
# (실행 가능 전체 예제: Evaluator_Examples/ch08_group_e.py 참고)
from agent_evaluator import agent_eval, SecurityConfig
...
```python
# 개념 코드 — InstructionConfig·SLAConfig 단계별 통합 패턴
# (InstructionConfig·SLAConfig·ThreatSeverityConfig는 ch12_decorators.py에 없음 —
#  실행 가능 예제: Evaluator_Examples/ch03_harness_basics.py 참고)
...
from agent_evaluator import RetryConfig, LLMJudgeConfig, SecurityConfig
...
```python
# 개념 코드 — PerformanceMonitor + @agent_eval + HarnessEvaluationGate 통합 패턴
# (InstructionConfig·SLAConfig·ThreatSeverityConfig는 ch12_decorators.py에 없음 —
#  실행 가능 예제: Evaluator_Examples/ch03_harness_basics.py 참고)
from agent_evaluator import (
...
from agent_evaluator import agent_eval, RetryConfig, LLMJudgeConfig, SecurityConfig
...
eval_q = QuickEval.for_security("results/")  # Gate E 강화 설정

@eval_q.qa
def simple_agent(question: str, ground_truth: str = "") -> str:
    return llm.invoke(question)

for q, gt in test_dataset:  # test_dataset은 위에서 정의됨
...
monitor = PerformanceMonitor(output_dir="results/", use_korean_tokenizer=True)  # 파일 상단 전역 생성
