...
Agent Evaluator v0.9.7 기준. 24개 통합 프레임워크와 8개 평가 플랫폼 비교를 정리한다.
...
| `vertexai` | 별도 설치 | ✅ | ❌ | ✅ | ❌ | ❌ |
| `openai_agents` | 별도 설치 (`openai-agents`) | ✅ | ❌ | ✅ | ❌ | ❌ |
| `google_adk` | 별도 설치 (`google-adk`) | ✅ | ❌ | ✅ | ❌ | ❌ |
| `claude_agent_sdk` | 별도 설치 (`claude-agent-sdk`) | ✅ | ❌ | ✅ | ❌ | ❌ |
...
**자동 감지**: `auto_detect_framework=True` (기본 비활성 — 명시 활성화 필요) 설정 시 12개 속성 기반으로 프레임워크를 자동 감지한다. 오감지가 의심될 경우 `framework=` 파라미터로 명시적으로 지정한다. **`openai_agents`/`google_adk`/`claude_agent_sdk` 3종은 자동 감지 대상에 포함되지 않는다** — 기존 프레임워크와의 속성 충돌 위험(예: Claude Agent SDK의 `AssistantMessage`와 raw Anthropic `Message`)을 검증 없이 감수하지 않기 위한 설계 선택이며, 이 3종은 반드시 `framework=`를 명시해야 한다.
...
| **Agent Evaluator** | **$0** (네이티브 25개) | **✅** | **밀리초** |
...
아래 기능들은 `pip install agent-evaluator` 기본 설치에 포함됩니다.
...
# agent_eval은 asyncio.iscoroutinefunction()으로 async 함수를 자동 감지한다
