...
**방법 1: 규칙 기반 (Agent-Evaluator HallucinationDetector — Gate C + G 폴백)**
...
| Anthropic 키 없음 | `gpt-5-nano` | OpenAI 자동 선택 (기본 fallback) |

```python
# 개념 코드 — judge_model=None 시 자동 선택 로직 (실제: agent_evaluator/integrations/llm_judge.py)
def _auto_select_model():
    if os.getenv("ANTHROPIC_API_KEY"):
        return "claude-haiku-4-5-20251001"  # Anthropic 우선
    elif os.getenv("OPENAI_API_KEY"):
        return "gpt-5-nano"                 # OpenAI 폴백
    else:
        return "gpt-5-nano"                 # 키 없을 때도 gpt-5-nano fallback (오류 없음)
```
...
```
HallucinationRate (Gate C + G 폴백, 규칙 기반):
  - 높을수록 나쁨: 0.0 = 완벽, 1.0 = 최악
  - 컨텍스트 없이도 측정 가능 (ground_truth 기반)
  - 수치 불일치와 미지원 주장 탐지
  - enable_hallucination_detection=True 시 Gate C·G의 폴백 지표로 기여 (1 − rate)

Faithfulness (LLM Judge, Gate C):
...
| 외부 의존성 없이 평가 | Gate C HallucinationDetector |
| 가장 정밀한 RAG 평가 | Ragas 4지표 모두 |
| 빠른 배포 전 검사 | LLM Judge (`judge_sample_rate=0.1`) |
| 컨텍스트 없는 환경 | Gate C 기반만 (Ragas/LLM Judge 모두 context 필요) |
...
| Hallucination (Gate C + G 폴백) | 5~20ms | 없음 | 최소 |
...
| # | 지표 | Gate | opt-in | 외부 의존 | 속도 | 비용 | 한국어 지원 | 주요 용도 |
|---|------|------|--------|----------|------|------|----------|----------|
| 1 | TCR | A | — | 없음 | ⚡ | $0 | ✅ | 핵심 완료율 |
| 2 | 정확도 | A | — | 없음 | ⚡ | $0 | ✅ | 응답 품질 |
| 3 | 응답 품질 | A | — | 없음 | ⚡ | $0 | ✅ | 다차원 품질 |
| 4 | 지연시간 | D | — | 없음 | ⚡ | $0 | N/A | 성능 SLA |
| 5 | 토큰 경제 | — | — | 없음 | ⚡ | $0 | N/A | 비용 추적 |
| 6 | 환각 탐지 | C + G | ✅ | 없음 | ⚡ | $0 | △ | RAG 품질 |
| 7 | 도구 효율 | G | — | 없음 | ⚡ | $0 | N/A | 도구 패턴 |
| 8 | 재시도·복구 | — | — | 없음 | ⚡ | $0 | N/A | 오류 회복 |
| 9 | 도구 선택 F1 | F | — | 없음 | ⚡ | $0 | N/A | 도구 정확도 |
| 10 | 에이전트 협력 | F | — | 없음 | ⚡ | $0 | N/A | 멀티에이전트 |
| 11 | 워크플로 실행 | — | — | 없음 | ⚡ | $0 | N/A | 프로세스 품질 |
| 12 | 입력 위생화 | E | ✅ | 없음 | ⚡ | $0 | ✅ | 주입 공격 |
| 13 | 출력 누출 | E | ✅ | 없음 | ⚡ | $0 | ✅ | 데이터 보호 |
| 14 | 도구 인가 | E | ✅ | 없음 | ⚡ | $0 | N/A | 권한 관리 |
| 15 | 권한 상승 | E | ✅ | 없음 | ⚡ | $0 | N/A | 보안 위협 |
| 16 | 체인 공격 | E | ✅ | 없음 | ⚡ | $0 | N/A | 복합 공격 |
| 17 | LLM Judge (완결성) | — | ✅ | LLM API | 🐢 | $$ | ✅ | 주관적 품질 |
| 18 | LLM Judge (관련성) | A | ✅ | LLM API | 🐢 | $$ | ✅ | 질문 적합성 |
| 19 | LLM Judge (사실성) | — | ✅ | LLM API | 🐢 | $$ | ✅ | 팩트체크 |
| 20 | LLM Judge (독성) | — | ✅ | LLM API | 🐢 | $$ | ✅ | 안전성 |
| 21 | LLM Judge (편향) | — | ✅ | LLM API | 🐢 | $$ | ✅ | 공정성 |
| 22 | Faithfulness | C | ✅ | LLM API | 🐢 | $$ | ✅ | RAG 충실도 |
...
*본 Appendix는 Agent-Evaluator v0.9.7 기준이며 외부 서비스 가격은 2025년 기준이다. 최신 가격은 각 서비스 공식 가격표를 참조하라.*
