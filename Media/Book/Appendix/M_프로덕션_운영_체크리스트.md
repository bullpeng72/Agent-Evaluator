...
# 점검 결과를 JUnit XML로 저장 (CI/CD 아티팩트)
agent-eval gate results/golden_eval.json \
  --tcr 85 --accuracy 70 \
  --junit-xml gate_check.xml
...
monitor = PerformanceMonitor(output_dir="results/", enable_hallucination_detection=True, use_korean_tokenizer=True)
...
from agent_evaluator import AlertEngine
...
*이 체크리스트는 Agent-Evaluator SDK v0.9.7 기준으로 작성되었다. 신규 Gate나 Config 추가 시 체크리스트를 함께 갱신한다.*
