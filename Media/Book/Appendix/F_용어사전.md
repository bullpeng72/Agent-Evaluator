...
Agent Evaluator v0.9.7에서 사용하는 주요 용어를 영문 기준 가나다 순으로 정리한다.
...
### AI Native 5속성 ↔ Harness Gate 매핑

| AI Native 속성 | 기존 테스팅의 한계 | 대응 Harness Gate |
|--------------|----------------|------------------|
...
| 형태 | 사용 상황 | 체크 항목 |
|------|----------|---------|
| `eval_q.gate(tcr=85, accuracy=70)` | 단순 배포 기준 (TCR·Accuracy) | 2개 지표 임계값 |
| `HarnessEvaluationGate(report).evaluate()` | Gate A–G 종합 판정 | 7개 Gate 전체 Config 위반 여부 |
...
```python
# 단순 Gate
eval_q.gate(tcr=85, accuracy=70)   # 미달 시 sys.exit(1)
...
**QA 관리자 관점**: Tracker가 수집한 데이터가 Gate A–G의 점수 원천이다. "Gate D 점수가 낮다"면 `LatencyTracker`가 SLA 초과를 탐지한 것이다. (`TokenEconomyTracker`는 gate score에 기여하지 않으며 비용 추적·보고 전용이다.)

| 종류 | Tracker | 담당 Gate | 활성화 |
|------|---------|-----------|--------|
| 기본 자동 | `TaskCompletionTracker`, `AccuracyEvaluator`, `ResponseQualityEvaluator` | A | 항상 |
| 기본 자동 | `LatencyTracker` | D | 항상 |
| 기본 자동 | `ToolCallAnalyzer` | G | 항상 |
| 기본 자동 | `AgentCoordinationTracker`, `ToolSelectionTracker` | F | 항상 |
| 기본 자동 | `TokenEconomyTracker`, `WorkflowExecutionTracker`, `RetryCorrectionTracker` | — (운영 추적) | 항상 |
| opt-in | `HallucinationDetector` | **C, G** | `enable_hallucination_detection=True` |
...
| 외부 통합 | `LLMJudge` (기본 5차원; RAG 시 +faithfulness, G-Eval 시 +커스텀 기준) | G | `enable_llm_judge=True` + API 키 |
...
2. **진단**: Gate별 Tracker 드릴다운으로 원인 귀속
...
> ⚠️ 이전 버전 문서에 Gate A로 잘못 기재된 경우가 있음. 정확한 Gate는 **C (신뢰성)**.
...
분산 시스템 관측가능성을 위한 오픈소스 표준. CNCF(Cloud Native Computing Foundation)가 관리하며 벤더 중립적 SDK와 프로토콜을 제공한다. Agent Evaluator에서는 `setup_otel()` 호출 후 `record_task()` 실행 시 OTLP 스팬을 자동으로 발행한다. 기본 설치에 포함되어 있어 별도 설치 불필요.
...
태스크 완료율. Gate A와 Gate C 모두에 기여하는 핵심 지표로 전체 태스크 중 `is_completed=True`(성공적으로 완료됨)로 표시된 태스크의 비율을 백분율로 표현한다. 95% 이상 우수, 85~95% 양호, 70% 미만 개선 필요.
...
스트리밍 응답에서 첫 번째 토큰이 생성되기까지의 시간. `LatencyTracker.track_ttft()`로 기록하거나 데코레이터 방식에서 제너레이터 함수의 첫 청크 yield 시점에 자동으로 기록된다.
...
```python
from agent_evaluator import TTFTVariabilityConfig, PerformanceMonitor, agent_eval
...
```python
from agent_evaluator import CostPredictabilityConfig, PerformanceMonitor, agent_eval
...
에이전트 고유의 행동 패턴을 측정하는 Layer 2(Agentic) 트래커들의 총칭. Tool Call Efficiency(Gate G), Retry & Error Recovery(Gate 미기여), Tool Selection Accuracy(Gate F), Agent Coordination(Gate F), Workflow Execution(Gate 미기여) 5종을 포함한다. LLM API 없이 알고리즘 기반으로 계산된다.
...
```python
eval_q = QuickEval.for_regression_eval("results/", baseline_file="baseline.json")
```
