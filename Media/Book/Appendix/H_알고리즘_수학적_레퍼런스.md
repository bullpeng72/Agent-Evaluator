...
Harness Gate 판정 (generate_report() 호출 시)
  ...
  Gate C  ← accuracy variance, retry 재현성, faithfulness (ReproducibilityConfig)
  Gate D  ← p95_ms, total_cost (SLAConfig·ResourceBudgetConfig)
  Gate E  ← threat_score (ThreatSeverityConfig·ComplianceConfig)
  Gate G  ← tool_call_success_rate, explainability_score (ExplainabilityConfig)
        ↓
...
예시 (10개 지연시간: [0.5, 0.7, 0.9, 1.1, 1.3, 1.5, 2.0, 2.5, 4.0, 8.0]):
  ...
  P95 = sorted[8.55] = 4.0 × 0.45 + 8.0 × 0.55 = 6.2초
  P99 = sorted[8.91] = 4.0 × 0.09 + 8.0 × 0.91 ≈ 7.64초
```
...
**Harness Gate 연결**: `enable_hallucination_detection=True` 설정 시 `hallucination_rate`는 **Gate C (신뢰성)** 와 **Gate G (관측성)** 의 폴백 지표로 사용된다 (`1 − rate`). LLMJudge의 `faithfulness` 점수가 있으면 Gate C에 우선 적용되어 `hallucination_rate` 폴백을 대체한다. RAG 에이전트의 환각 제어가 목표라면 `LLMJudge(rag_mode=True)`의 `faithfulness` 점수를 주 신호로 사용할 것을 권장한다.
...
**Harness Gate 연결**: 도구 선택 F1은 **Gate F (Multi-Agent Coordination)** 점수에 직접 기여한다 (`ToolSelectionTracker`, `avg_f1_score / 100` 정규화). Gate B의 `ScopeConfig(allowed_actions=[...])` 와도 연관되어, 에이전트가 `allowed_actions` 외의 도구를 사용하면 Gate B의 범위 일탈(scope violation)로도 처리된다. Precision이 낮으면 불필요한 도구 호출이 많다는 신호다.
...
```python
# v0.9.7 기준: OWASP Top 10 for LLMs (2023) + MITRE ATLAS (2024) 기반
...
|----------------|-----------|-------------|
| `relevance` (관련성 점수 0–5) | Gate A | `GoalAlignmentConfig(use_llm_scoring=True, llm_blend_weight=0.5)` |
| `faithfulness` (RAG 모드) | Gate C | — (`enable_llm_judge=True` + RAG 모드 시 자동 기여) |
| `safety_score` (toxicity·bias 역수 변환) | — (Gate 미기여) | 로깅 전용 — Gate E는 5개 보안 Tracker로만 집계 |
| `criteria_overall` (커스텀 기준 평균) | — (Gate 미기여) | 로깅 전용 — Gate G `_obs_vals`에 포함되지 않음 |

`llm_blend_weight=0.5`는 규칙 기반 점수와 LLM Judge 점수를 50:50으로 혼합해 Gate 판정에 사용함을 의미한다. 값을 0으로 설정하면 LLM Judge를 로깅 전용으로만 사용한다.

---

*본 Appendix의 수식과 알고리즘은 Agent-Evaluator v0.9.7 소스 코드(`agent_evaluator/core/trackers/layer1.py`, `layer2.py`, `security.py`, `integrations/llm_judge.py`)와 직접 대응된다.*
