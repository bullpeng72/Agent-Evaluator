...
# baseline_window=25: 앞 25건을 기준선으로, detection_window=5: 마지막 5건(이상 케이스)을 현재값으로 비교
monitor_anomaly = PerformanceMonitor(output_dir=_OUTPUT_DIR)
detector = AnomalyDetector(baseline_window=25, detection_window=5)

# 정상 기준선 데이터 (30건) → monitor_anomaly에 순서대로 기록
# ⚠️ accuracy_drift 탐지 조건:
#   _check_accuracy_drift() → Z-score > 2.5 AND drift > 15%
#   baseline std = 0 이면 z-score 계산 불가 → 탐지 불가
#   → 기준선에 다양한 질문/응답 쌍 사용하여 자연스러운 std > 0 생성
_BASELINE_QA = [
    # (question, response, ground_truth) — 부분 일치로 acc 0.5~0.9 분포
...
# 이상 패턴 주입 (5건) → 모두 monitor_anomaly에 기록한 뒤 한 번에 스캔
#
# 탐지 결과 예상:
#   ✅ latency_trend  — 끝에 큰 지연 스파이크, 선형회귀 기울기 > 0.05
#   ✅ token_spike    — IQR 기준선 대비 +800% 이상 토큰 폭증
#   ✅ error_surge    — 빈 응답 3건 → 오류율 60%  (기준선 0%)
#   — accuracy_drift  — 탐지 안 됨: Z-score > 2.5 + drift > 15% 두 조건 동시 충족 어려움
#                       (baseline variance가 낮으면 z-score 미달, 높으면 최소 드리프트 미달)
ANOMALY_CASES = [
    # (label, execution_time, response, tokens)
    ("오류 급증1",   1.2,  "",     140),   # response="" → accuracy=0, error
    ("오류 급증2",   1.1,  "",     130),   # response="" → accuracy=0, error
    ("토큰 폭증",    1.5,  "응답", 5000),  # 토큰 IQR 이상치
    ("오류 급증3",   1.3,  "",     145),   # response="" → accuracy=0, error  → error_surge 60%
    ("지연 스파이크", 15.0, "응답", 150),  # 마지막 큰 spike → slope 양수 → latency_trend
]
...
# 전체 35건(기준선 30 + 이상 5) 기록 후 한 번에 스캔
# scan()은 PerformanceMonitor 인스턴스를 받아 내부 태스크 이력에서 이상을 탐지한다
anomaly_events: list = []
try:
    ...
    print(f"  스캔 완료 (이벤트 없음 또는 데이터 부족)")
...
for model, tokens in MODEL_USAGES:
    ...
    # CostTracker.record()를 직접 호출해야 get_today_cost()에 반영됨
    ...
    # get_status() 반환 키: "stage" (not "current_stage")
