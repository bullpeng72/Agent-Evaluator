"""
05_streaming_alerts.py — 실시간 스트리밍 평가 + 알림 엔진
==========================================================
...
결과:
    results/05_streaming_alerts.json
"""
...
try:
    ...
    with socket.socket() as s:
        ...
        if s.connect_ex(("localhost", 6006)) == 0:
            setup_otel(endpoint="http://localhost:6006", service_name="05-streaming-alerts")
...
monitor.save_to_file("05_streaming_alerts")
print("\n결과 저장 완료: results/05_streaming_alerts.json")

# 저장 확인
saved = json.loads(Path(_OUTPUT_DIR, "05_streaming_alerts.json").read_text())
