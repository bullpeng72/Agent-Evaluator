"""
07_phoenix_hybrid.py — Phoenix OTEL 완전 통합 + DeepEval/Ragas (외부평가)
========================================================================
...
Layer 3 외부 통합 — 대시보드 '외부평가' 탭 활성화:
  OPENAI_API_KEY + pip install 'agent-evaluator[eval]' 설치 시:
    ...
    → results/07_phoenix_hybrid.json 에 rag_metrics·advanced_metrics_summary 포함
...
결과:
    results/07_phoenix_hybrid.json
"""
...
if PHOENIX_ONLINE:
    ...
    setup_otel(endpoint=_PHOENIX_URL, service_name="07-phoenix-hybrid")
...
# Variables: {"proj": "07-phoenix-hybrid"}
...
if hasattr(monitor, "save_to_file"):
    monitor.save_to_file("07_phoenix_hybrid")
...
if not EVAL_AVAILABLE:
    ...
    # 저장된 JSON 읽기
    json_path = Path(_OUTPUT_DIR) / "07_phoenix_hybrid.json"
    try:
...
if PHOENIX_ONLINE:
    print(f"\nPhoenix 확인:")
    print(f"  Tracing    → {_PHOENIX_URL}  (서비스: 07-phoenix-hybrid)")
