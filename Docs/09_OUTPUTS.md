...
| 3 | Gate A–G 상세 (`gate-a`…`gate-g`) | L3 | **Score Breakdown** (산식 + 컴포넌트별 raw value/기여도/note) + KPI + 상세 표. 측정 컴포넌트 ≤2 & score<90이면 "대표성 낮음" 경고. **전 Gate `insufficient_data_warnings`**(표본 부족 컴포넌트) 노출 | 항상 (미측정 항목은 `⚙️ Not Configured` / `📉 Insufficient Data` / `➖ Not Applicable` 배너) |
...
| 6 | **실패/저점 케이스** (`failure-cases`) | **L1** | ① **Failure set vs baseline** — 📉Regressed/♻️Persistent/🆕New/✅Fixed (baseline 시) ② **Failure themes** — `(사유 테마 × task_type)` 군집을 count·영향도(~%p) 순 ③ **Worst cases** 표 (`Status · Question→Response · Score(C/A) · Likely reason`) | 실패·저점 태스크 있을 때 |
