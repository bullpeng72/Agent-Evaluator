...
| 4b | **Multi-Turn Conversation** (`conversation`) | **L5** | 턴별 context-reference 궤적(스파크라인) + `degradation_after_turn`(에이전트가 답을 포기하기 시작하는 턴) + `goal_drift_sessions`(세션이 주제 이탈) + worst_session (SPEC-041 P24) | `conversation_sessions[]` 있을 때 |
