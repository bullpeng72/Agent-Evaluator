...
| Tool | Input | Output |
|------|-------|--------|
| `list_violations(limit=20, since=None, gate=None)` (SPEC-045 REQ-5, v1.1.0) | nothing required | the keyword-free discovery entry point — most-recent-first blocked/observed history, no query needed. Same rendering as `search_violations` (`[blocked]`/`[observed]` prefix, captured excerpt inline); use when you don't already know what to search for |
