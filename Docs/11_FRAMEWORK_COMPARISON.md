...
| **Agent Evaluator** | OSS SDK | v0.7.5 (2026.04) | Agentic AI 전문 평가 |
...
| **Agent Evaluator** | ✅ OTLP Exporter (v0.7.0) | Span Exporter | `setup_otel()` + `record_task()` 시 자동 스팬 발행. Phoenix 연동 |
...
> **v0.7.5 기준**: LangChain ≥1.0 / LangGraph ≥1.0 / CrewAI ≥1.0 / AutoGen 0.4+ (async-first) 최신 API 완전 지원. `@agent_eval(framework="langchain")` 형태로 21개 프레임워크 메타데이터 자동 추출. 4개 프레임워크 모두 16개 네이티브 지표 + 9개 Layer3 하이브리드 = 25개 지표 체계 적용 (프레임워크별 커버리지: LangChain ~82%, LangGraph ~82%, AutoGen ~80%, CrewAI ~78%).
