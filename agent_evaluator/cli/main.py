...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            f"  {Y}decisions{R}    Review / record human decisions on gate runs (deploy ledger)\n"
            ...
            f"  {Y}dataset{R}      Golden datasets: build / promote / health / review-candidates\n"
            f"  {Y}feedback{R}     Export A/B preference rows as a JSONL dataset\n"
            ...
            f"  {Y}improve{R}      Closed loop: plan / start / verify / patch / apply-verify\n"
            f"  {Y}claims{R}       Manage .aoo/claims.jsonl team scope claims\n"
            f"  {Y}autopilot{R}    Harness Autopilot — HITL approval queue on .aoo/ "
            "(tasks/team/phase/approvals/decisions/skills, + a local dashboard)\n"
            f"  {Y}opencode{R} · {Y}claude{R}  LiveGuardrail plugin / CLI hooks —\n"
            f"               install / upgrade / doctor / test-config / uninstall / violations\n"
            "\n"
            f"{B}Examples:{R}\n"
            ...
            f"  {G}agent-eval autopilot install --platform ac{R}\n"
            f"  {G}agent-eval autopilot dashboard{R}\n"
