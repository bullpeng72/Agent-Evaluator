...
class InstructionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.violation_weight < 0.0:
            _w.warn(
                f"InstructionConfig: violation_weight={self.violation_weight} < 0 이므로 "
                f"기본값 0.1로 보정됩니다. 음수 가중치는 위반이 많을수록 점수가 오히려 "
                f"오르는 역전을 유발합니다.",
        ...
        if _neg:
            _w.warn(
                f"InstructionConfig: violation_weights에 음수 값이 있습니다: {_neg}. "
                f"음수 가중치는 해당 위반 유형이 감지될수록 점수가 오히려 오르는 역전을 유발합니다.",
                UserWarning, stacklevel=2,
            )
...
class GoalAlignmentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0.0 <= self.llm_blend_weight <= 1.0):
            _w.warn(
                f"GoalAlignmentConfig: llm_blend_weight={self.llm_blend_weight} 은 [0.0, 1.0] "
                f"범위를 벗어납니다. 사용 시점에 자동으로 클램핑되지만, 실제 적용값이 설정값과 "
                f"달라집니다(0.0=규칙 기반만, 1.0=LLM만).",
                UserWarning, stacklevel=2,
            )
        if not (0.0 <= self.alignment_threshold <= 1.0):
            _w.warn(
                f"GoalAlignmentConfig: alignment_threshold={self.alignment_threshold} 은 "
                f"[0.0, 1.0] 범위를 벗어납니다 — 정렬 점수는 0-1로 정규화되므로 "
                f"범위 밖 임계값은 항상 통과 또는 항상 경고를 유발할 수 있습니다.",
                UserWarning, stacklevel=2,
            )
    ...
    def __post_init__(self) -> None:
        ...
        if self.min_steps > self.max_steps:
            _w.warn(
                ...
                f"어떤 계획 단계 수도 두 조건을 동시에 만족할 수 없어 항상 위반으로 판정됩니다.",
                UserWarning, stacklevel=2,
            )
        if self.min_steps < 0:
            _w.warn(
                f"PlanConfig: min_steps={self.min_steps} < 0 이므로 0으로 보정됩니다.",
                UserWarning, stacklevel=2,
            )
            self.min_steps = 0
        if not (0.0 <= self.llm_blend_weight <= 1.0):
            _w.warn(
                f"PlanConfig: llm_blend_weight={self.llm_blend_weight} 은 [0.0, 1.0] 범위를 "
                f"벗어납니다. 사용 시점에 자동 클램핑되지만, 실제 적용값이 설정값과 다릅니다.",
                UserWarning, stacklevel=2,
            )
...
class ContextRetentionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        for _name, _val, _default in [("entity_weight", self.entity_weight, 0.6),
                                       ("goal_weight", self.goal_weight, 0.4)]:
            if _val < 0.0:
                _w.warn(
                    f"ContextRetentionConfig: {_name}={_val} < 0 이므로 기본값 {_default}로 "
                    f"보정됩니다. 음수 가중치는 결합 점수의 부호를 반전시킬 수 있습니다.",
                    UserWarning, stacklevel=2,
                )
        ...
        if not (0.0 <= self.retention_threshold <= 1.0):
            _w.warn(
                f"ContextRetentionConfig: retention_threshold={self.retention_threshold} 은 "
                f"[0.0, 1.0] 범위를 벗어납니다.",
                UserWarning, stacklevel=2,
            )
        if not (0.0 <= self.goal_overlap_threshold <= 1.0):
            _w.warn(
                f"ContextRetentionConfig: goal_overlap_threshold={self.goal_overlap_threshold} 은 "
                f"[0.0, 1.0] 범위를 벗어납니다.",
                UserWarning, stacklevel=2,
            )
...
class SubtaskConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0.0 <= self.min_completion_rate <= 1.0):
            _w.warn(
                f"SubtaskConfig: min_completion_rate={self.min_completion_rate} 은 [0.0, 1.0] "
                f"범위를 벗어납니다. completion_rate(0-1)와 비교되므로 음수면 항상 통과, "
...
class KnowledgeRetentionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0.0 <= self.retention_threshold <= 1.0):
            _w.warn(
                f"KnowledgeRetentionConfig: retention_threshold={self.retention_threshold} 은 "
                f"[0.0, 1.0] 범위를 벗어납니다.",
                UserWarning, stacklevel=2,
            )
        if self.seed_turns < 0:
            _w.warn(
                f"KnowledgeRetentionConfig: seed_turns={self.seed_turns} < 0 이므로 0으로 "
                f"보정됩니다. 음수 값은 conversation_history[:seed_turns] 슬라이싱에서 "
                f"'끝에서부터 N개 제외'로 해석되어 의도와 다른 턴이 시드로 쓰입니다.",
                UserWarning, stacklevel=2,
            )
            self.seed_turns = 0
        ...
        if self.check_from_turn < self.seed_turns:
            _w.warn(
                ...
                f"seed_turns={self.seed_turns}. 시드로 사용한 턴이 검사 대상에도 포함되어 "
                f"'시드 이후 유지 여부' 측정 의도와 어긋날 수 있습니다.",
                UserWarning, stacklevel=2,
            )
