...
class ObservabilityConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0.0 <= self.min_coverage <= 1.0):
            _w.warn(
                f"ObservabilityConfig: min_coverage={self.min_coverage} is outside the "
                f"[0.0, 1.0] range. It is compared against trace_coverage (0-1), so a negative "
                f"value always passes the SLO and a value above 1.0 always fails.",
                UserWarning,
                stacklevel=2,
            )
...
class ExplainabilityConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.min_reasoning_length < 0:
            _w.warn(
                f"ExplainabilityConfig: min_reasoning_length={self.min_reasoning_length} < 0; "
                f"clamping to the default 20. A negative value always passes the length check, "
                f"inflating Gate G.",
                UserWarning,
                stacklevel=2,
            )
...
class ErrorDiagnosisConfig:
    ...
    failure_acknowledgment_markers: list[str] = dataclasses.field(
        default_factory=lambda: [
            "failed",
            "unable",
            "error",
    ...
    root_cause_markers: list[str] = dataclasses.field(
        default_factory=lambda: [
            "because",
            "due to",
            "caused by",
    ...
    suggestion_markers: list[str] = dataclasses.field(
        default_factory=lambda: [
            "try",
            "suggest",
            "recommend",
    ...
    def __post_init__(self) -> None:
        ...
        for _attr, _default in [
            ...
            if getattr(self, _attr) < 0.0:
                _w.warn(
                    f"ErrorDiagnosisConfig: {_attr}={getattr(self, _attr)} < 0; "
                    f"clamping to the default {_default}. A negative weight makes diagnosis_score "
                    f"negative, distorting the Gate G score.",
                    UserWarning,
                    stacklevel=2,
                )
        ...
        if (self.acknowledgment_weight + self.root_cause_weight + self.suggestion_weight) == 0.0:
            _w.warn(
                "ErrorDiagnosisConfig: the weights (acknowledgment_weight + root_cause_weight + "
                "suggestion_weight) sum to 0.0. diagnosis_score will always be 0.",
                UserWarning,
                stacklevel=2,
            )
...
class LatencyAttributionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        for _attr, _default in [
            ...
            if not (0.0 <= _val <= 1.0):
                _w.warn(
                    f"LatencyAttributionConfig: {_attr}={_val} is outside the [0.0, 1.0] range. "
                    f"Clamping to the default {_default}. "
                    f"If > 1.0 that penalty is always 0, inflating Gate G; "
                    f"if < 0.0, even a fully-attributed task is penalized, deflating Gate G.",
                    UserWarning,
                    stacklevel=2,
                )
                setattr(self, _attr, _default)
