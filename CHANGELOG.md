...
- ✨ **Multi-task / multi-team registry, full CRUD** — `.aoo/tasks/<id>.json` + `.aoo/team.json`, AC/AOO platform-neutral. `install` / `doctor` / `new-task` / `add-member`, plus full CRUD: `remove-member` / `update-member` / `update-task` / `list-members` / `list-tasks` / `show-task` / `set-task-status`. The dashboard board hides archived/cancelled tasks by default (`?show_all=1` to include them) and its "+ 새 과제" form takes all 6 owner roles (not just 2), matching the CLI. `update_task()`/`set_task_status()`/`update_team_member()` all accept an optional `changed_by` — previously none of the three recorded who made a change (unlike `transition_phase()`'s `approved_by`/`decide_approval()`'s `decided_by`); stored as `last_updated_by`/`last_updated_at` (+ `status_changed_by` for status changes), with a matching CLI `--by NAME` flag and a dashboard "수정자"/"변경자" field. The same three also accept an optional `expected_updated_at` for optimistic concurrency — two people editing the same task/member from the dashboard at once would otherwise have the second save silently overwrite the first; a stale value now raises instead (surfaced through the dashboard's error-banner mechanism), reusing a timestamp these functions already keep rather than new lock infrastructure. `remove_team_member()` is deliberately left out of the actor-attribution change — it already documents `team.json` as a history-less snapshot (use git if you need one).
- ✨ **Local dashboard** (`agent-eval autopilot dashboard`, port 8766) — task board, team management, HITL approval queue, phase policy, and an ops view (claims / decisions / rejection-rate), server-rendered against real `.aoo/` files (no JS). Has full write parity with the CLI: creating/editing/archiving a task, editing/removing a team member, opening/deciding/updating/cancelling an approval and transitioning its task's phase, viewing/setting/clearing the phase policy, opening/releasing a claim, recording a deploy decision, and running the `scan-thresholds` sweep are all reachable from the dashboard, not just the CLI. The task detail page shows `phase_history` (who transitioned to which phase, and when) in a "Phase 이력" table — previously only CLI `show-task` displayed it, even though `transition_phase()` records it on every call. A task's `blocking_on` field is computed live from the approval queue (it was never updated by anything before, so the board's "pending approval" badge never lit up). 10 dashboard write routes that used to silently swallow a `ValueError` and redirect back with zero feedback (duplicate task/member id, unknown task/member/approval id, invalid status/platform/role/checklist-status/phase-policy kind, already-decided approval) now show the same `.banner.critical` pattern the other 3 routes already had (`task_error`/`status_error`/`update_error`/`team_error`/`action_error`/`policy_error`, depending on the page).
- ✨ **HITL approval queue** (`.aoo/approvals.jsonl`) — `approvals {open,list,decide,update,cancel}`. Checklist auto-scoring + `[NEEDS CLARIFICATION: ...]` tag detection keep a draft out of the human queue until it's ready; `--checklist-item "LABEL:STATUS"` splits on the **last** colon (a label containing its own colon no longer truncates and silently blocks forever) and rejects an invalid `STATUS` immediately. `required_approvals` (default 2 for `deploy`/`release_hold`) enforces dual sign-off by two distinct approvers — the dashboard's decide form used to hard-code `decided_by="local-reviewer"`, making that structurally impossible through the UI; it's now a real input. The open-approval form (CLI `--required-approvals N` / dashboard field, blank keeps the kind's default) can escalate a normally single-approval kind to dual sign-off for one specific request. `approvals update` fixes a draft's checklist without redrafting from scratch; `approvals cancel` withdraws a `draft`/`pending` request (new terminal status `cancelled`, excluded from the rejection-rate denominator — a withdrawal isn't a rejection). `approvals list` (pending-only by default) now nudges when draft approvals exist but are hidden.
- ✨ **`approvals scan-thresholds`** — detects a repeated `--hold-on-undecided` exit-75 reason (5+ occurrences) in `.aoo/decisions.jsonl` and idempotently opens a `threshold_review` approval (4-step checklist, `--no-checklist-gate` if it shouldn't gate "ready for review" — the internal `open_threshold_reviews()` used to always pass `gate_on_checklist=False` with no way to override it for a manually-opened kind). Deduplicates on the repeated reason itself rather than a title string with the count baked in (previously the same card could duplicate once a reason recurred past its original count). No new scoring — reads the existing decision ledger only. Also runnable from the approvals page (a "스캔 실행" form).
- ✨ **`skills detect` / `skills scaffold`** — read-only detection of repeated approval-checklist shapes as skill candidates (counted per distinct `task_id`, so a checklist mistake redrafted several times doesn't look like a repeated cross-task pattern), plus `scaffold --name NAME` to write a `Skills/<name>/SKILL.md` stub from a candidate (TODO markers left for a human; never auto-finished). `skills detect`'s candidates now also show up in a read-only card on the dashboard's approvals page; `skills scaffold` (which writes a file) stays CLI-only by design — creating a skill is still a deliberate human step.
- ✨ **`phase transition --require-approval KIND`** — opt-in gate that refuses a task's phase transition unless an approval of that kind is `approved` for that task (the task-existence check now runs before this, so transitioning a nonexistent task gives the right error instead of a misleading "no approved approval" one). Warns (never blocks) on a backward or skipped transition. `phase policy set --to N --require-approval KIND` declares this once per project instead of remembering the flag on every call; the dashboard's phase-transition form now consults this policy too (it used to ignore `.aoo/phase_policy.json` entirely, silently bypassing a declared project policy whenever someone transitioned a phase from the dashboard instead of the CLI) — the task detail page shows the active policy as a hint, and the ops page gets a Phase 정책 card to view/set/clear it. `doctor --stale-days N` / standalone `phase check` flag a task that has sat in its current phase too long — the first signal that a task's declared phase may have silently fallen behind the actual work; the board's stale badge and the task detail page's stale banner take a matching `?stale_days=` dashboard param (previously hardcoded to 7.0).
- ✨ **`phase transition --require-gate-ready {yes,no}`** — a second, independent opt-in phase gate alongside `--require-approval`, this time checking the SDK's own Harness Gate A–G verdict instead of a HITL approval. Until now, Autopilot's phase gate could only ask "did a human approve this?" — the actual Gate readiness verdict recorded in `.aoo/decisions.jsonl` (via `gate --decision-log`) was never consulted, so a task could be pushed all the way to phase 8 (운영) even with the latest gate run at `verdict_level="not_ready"`. This checks the most recent `gate_run`: passes if its `verdict_level` is `"ready"` (and a human hasn't explicitly `rejected`/`held` it), or if a human has explicitly `accepted`/`overridden` it regardless of verdict — the human's final call always wins over the automated verdict, in either direction. No new scoring: reads `summarize_decisions()`'s existing `last` gate-run/outcome pairing. `phase policy set --to N --require-gate-ready` / `--clear-gate-ready` declares it once per project (independent of the approval-kind policy — a phase can have either, both, or neither); the dashboard's phase-transition form and the ops page's phase-policy card both got matching controls. Along the way, `set_phase_policy()` — which used to overwrite `.aoo/phase_policy.json` with only its own `gates` key — was fixed to preserve the rest of the file, since adding a second policy axis (`gate_ready`) to the same file would otherwise have one axis silently erase the other every time it was written.
- ✨ **`agent-eval autopilot decisions {list,record}`** — the deploy-decision ledger commands, reachable under the `autopilot` tree (`--log` defaults to `.aoo/decisions.jsonl`); the original top-level `agent-eval decisions ...` still works unchanged. `decisions record` now requires an explicit `--gate-run-id` whenever 2+ gate runs are pending, instead of silently picking "the latest." The ops page has a matching "결정 기록" form (previously read-only), populated with the pending gate run(s) to choose from, plus a "Gate 점수" column showing each row's Gate A–G scores inline (raw values, `—` for an unmeasured gate) — that data has been in `gate_scores` on every `gate_run` entry since `--decision-log` shipped, it just was never rendered on the dashboard.
- ✨ **Claims lifecycle** — `claims add/list/release/audit` plus `agent-eval claims enable-live-check --config PATH` (merges a `{"team_concurrency": {...}}` block into an existing guardrail config JSON via a safe deep-merge, never overwriting a key already set); `claims add` warns (non-blocking) on scope overlap with an existing active claim. The ops page can open/release a claim directly (`append_claim()` didn't create `.aoo/` if it didn't exist yet — the CLI already worked around this with `mkdir`; the dashboard route does the same), shows the same TTL-exceeded / overlapping-scope violations `claims audit --ttl-hours` catches (previously CLI-only), and can filter the claims table to one developer via `?claim_developer=` matching CLI's `--developer NAME` (display-only — the TTL/overlap audit still always runs against every active claim, since that's a project-wide signal, not a per-developer one).
...
- ✨ **10 more Skills backported into `Skills/`** (now 15 total): `harness-gate-ci`, `claims-audit-ci`, `abtest-decision-workflow`, `gate-improvement-loop`, `recommend-fix-workflow`, `spec-driven-artifacts`, `tdd-ai-workflow-checklist`, `liveguardrail-standard-config` (from the Harness Methodology book's procedural skill set) plus `config-silent-noop-check`, `eval-run-directory-hygiene` (surfaced during the AOO Stack workbook's real Autopilot use) — each wraps an `agent-eval` CLI workflow (gate CI, claims audit CI, A/B-test decisions, the improve/recommend-fix loop, spec-driven artifacts, TDD-with-AI checklists, LiveGuardrail config, silent-noop config checks, eval-run directory hygiene) and is host-neutral (works the same under `agent-eval opencode` and `agent-eval claude`, since both wrap the same CLI layer). The book's `human-approval-loop` skill (intentionally not backported — its DIY approval loop is now superseded by Autopilot's own `.aoo/approvals.jsonl` + `decide_approval()`) gained a note pointing to Autopilot as the primary path, keeping the DIY template only for projects without Autopilot installed.
- 🐛 **Packaging fix: `Skills/` is now bundled in a real `pip install`.** `agent-eval autopilot install`'s skill-copy step resolved its source path via `Path(__file__).resolve().parents[2] / "Skills"` — correct only for an editable checkout (`pip install -e .`), where the repo's directory layout is preserved. A real, non-editable install places `agent_evaluator/` directly under `site-packages/` with no sibling `Skills/` directory, so every real `pip install` user got the skill-copy step silently finding nothing. Fixed by shipping `Skills/` as package data (`agent_evaluator/skills/`, via `pyproject.toml`'s wheel `force-include` + a matching sdist `include`) and resolving it through `importlib.resources` first, falling back to the repo-root `Skills/` for editable/dev checkouts (`_resolve_skills_root()`). Verified against a real (non-editable) wheel install in a throwaway venv, not just unit tests.
- ✨ **`agent-eval autopilot install` now installs every bundled skill**, not just `harness-autopilot` — and a new `agent-eval autopilot skills install [NAME|--all] [--platform ac|aoo]` subcommand re-fetches one skill (or the full set) into an existing project after the fact, e.g. once an SDK upgrade ships new skills.
...
- A fully-blocked tool call now keeps a short PII-redacted excerpt of its arguments (`blocked_attempt_capture`, on by default; `{"enabled": false}` restores the old behaviour). So `search_violations "rm -rf"` matches on the **command itself**, not just the generic `dangerous tool parameters` reason.
- New `show_violation(task_id)` MCP tool — search results chain into it automatically — plus `agent-eval {claude,opencode} violations [--detail]` / `blocked-detail <task_id>` CLI. Best-effort host-transcript fallback for sessions recorded before capture.
- `agent-eval claude doctor` / `agent-eval opencode doctor` gain a "blocked-attempt capture" live check.
- `blocked_violations` gains an indexed `arg_excerpt` column, auto-migrated in place from the old shape (no `schema_version` bump).

35 new tests (both hosts); full suite 4,976 passing.
...
- 🔧 **`arize-phoenix` pin is now Python-version-scoped** (`[otel]` / `[sdk]` / `[examples]` / `[full]`):
  - **3.12+** → `>=15.4.0,<21.0.0` — admits Phoenix v20 (v20's breaking changes — inferences / embeddings / UMAP removal — don't touch the SDK's OTLP + annotation surface); with `openai<3.0.0` the resolver lands on `arize-phoenix 20.3.x`.
  - **3.10 / 3.11** → `>=15.4.0,<19.0.0` — Phoenix **19.11+ and all 20.x require Python 3.12**: `phoenix.trace.dsl.filter._FilterBindings` is a frozen dataclass whose `boolean_names` field defaults to `MappingProxyType({})`, which CPython < 3.12 rejects as a "mutable default" at import — the Phoenix server / `agent-eval monitor` crash on startup. (`MappingProxyType` became hashable in 3.12, gh-87995.)
  - **3.8 / 3.9** → the line drops entirely (`arize-phoenix>=17` needs `arize-phoenix-otel`, which requires Python ≥3.10); only `opentelemetry-*` install. The SDK imports `opentelemetry.*`, never `phoenix`, so `setup_otel()` to a remote / Docker Phoenix or any other OTLP backend still works — this fixes a previously unresolvable `pip install "agent-evaluator[otel]"` on 3.9.
- ✨ **Phoenix annotation delivery — span + trace + session tiers.** `save_to_file()` now also POSTs `/v1/trace_annotations` (per-task) and `/v1/session_annotations` (one rollup row per session: `session_accuracy` / `session_completion` / `session_pass_rate` / `session_latency_s` / `session_hallucination` / `session_quality`), so Phoenix's "Trace / Session annotation scores" panels populate. `annotator_kind` is now `CODE` (deterministic computed values, was `LLM`).
- 🔧 **Annotation POST robustness.** Each POST uses `?sync=true` and retries HTTP 404 (span/trace/session not yet indexed) on an exponential backoff — default `0.5, 1, 2, 4` s, override with `AGENT_EVAL_PHOENIX_ANNOTATION_RETRY_DELAYS`. A one-time `GET {endpoint}/arize_phoenix_version` probe (cached per endpoint) skips the annotation POSTs entirely against a non-Phoenix OTLP endpoint, so `save_to_file()` no longer stalls on a plain collector. Traces/metrics are unaffected — they go out over standard OTLP.
- 🐛 **`agent-eval monitor` console output is now ASCII-only.** The connection box and the "Phoenix Tracing Auto-Refresh" block used box-drawing glyphs, an em-dash (ambiguous width — mis-aligned the box even in a UTF-8 terminal) and status emoji, which rendered as mojibake or raised `UnicodeEncodeError` on a non-UTF-8 console (e.g. cp949). Box now drawn with `+ - |`, status shown as `[ok]` / `[x]` / `[!]`. The 210-char browser-console auto-reload snippet was moved out of stdout (it wrapped into an unreadable blob) into [`Docs/06_OBSERVABILITY.md`](Docs/06_OBSERVABILITY.md); `monitor` now prints a two-line "press F5 when a `[new] project` line appears" note. Applies across `monitor` / `--check` / `--reset` / `--sync-datasets` output.
- 📖 **New doc — [`Docs/10_OTEL_DATA_REFERENCE.md`](Docs/10_OTEL_DATA_REFERENCE.md).** Every span, attribute, child span, metric, and Phoenix annotation the SDK puts on the wire, plus the transport, model-name resolution, size limits, failure isolation, portability to non-Phoenix OTLP backends, and the Python-version constraints.
- ✨ **New example — [`Evaluator_Examples/ch32_ollama_realtime.py`](Evaluator_Examples/ch32_ollama_realtime.py).** Drives a **local Ollama model** (via stdlib `urllib`, no extra deps) through QA / RAG / tool-use / SLA / reliability / a failing case / a multi-turn conversation / a two-agent draft-review loop / a prompt-injection probe — real responses, real token counts (`prompt_eval_count` / `eval_count`), real latencies — with all 7 Gates + hallucination + security + anomaly detection on, then writes JSON + HTML and prints the dashboard / Phoenix follow-up. `model_name="ollama/<model>"` so Phoenix's LiteLLM table groups it under "Top models by tokens" (cost is $0 — accurate for local inference).
- 🐛 **`Evaluator_Examples/ch19_phoenix.py`** now sets `model_name` and includes a failing task + a failing tool call, so the Phoenix **Cost / Top-models / LLM-span-errors / Tool-span-errors** panels populate instead of staying empty.
